...
$stafile_exist='/Users/timlin/Documents/Project/NASA/LSTM_training/Chile/chile_GNSS/GMTout/Illapel2015/exist_sta.txt';
...
$timeseries='/Users/timlin/Documents/Project/NASA/LSTM_training/Chile/chile_GNSS/GMTout/Illapel2015/timeseries.txt';
...
#$area='-78/-65.5/-45.0/-17'; #range=28
$area='-78/-65.5/-35.0/-25.0'; #range=10, 28*0.3571426
$ratio=10/28;
$new_x0=-74; #plotmap and see where to set x0
$new_x1=$new_x0+12.5*$ratio;
print "newx0,x1=$new_x0, $new_x1\n";
$area='-74/-69.5357142857143/-45.0/-35.0'; #range=10 , range of orig_x=12.5 change the x0 to -74, x1=12.5*0.3571426=-69.53571
#set area_L='-125.0/45/40/50/8' #used by -JL
#$area_L='-71.0/-31/-15/-14/8'; #used by -JL
$area_L='M3i'; #used by -J
#$fileout="Cascadia_2_${run_num}";
#$EQID="000001";
#---Illapel---
$evlo=-71.674;
$evla=-31.573;
$scale_velo=4.0; #scale for vector plot
#-------------
...
`gmt grdimage $grdfile -I$grdgrad -J$area_L -R$area -Ctopo.cpt -Y2i -X1i -P -t70 -K  > $fileout".ps"`;
...
`gmt psxy Illapel_2m.xy -W1.2p,255/0/255 -R -J -O -K >>$fileout".ps"`;


#-----------plot prediction fault on map----------
$pred_fault = `ls /Users/timlin/TEST_MLARGE/Out1/fault_${epo}.txt`; chomp($pred_fault);
...
`gmt makecpt -Cseis -T-0.2/0.2/0.01 -Z -V0 > Z.cpt`; #change the boundary manually
...
$tmpscale=1*${scale_velo};
print PSVELO "-76.8 -25.6 $tmpscale 0 0 0 0\n";
close(PSVELO);
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
print PSTEXT "-76.3 -26.2 12 0 1 2 1m";
close(PSTEXT);
#---plot colorscale---
`gmt gmtset MAP_ANNOT_OFFSET_PRIMARY 0p MAP_LABEL_OFFSET 0.5p FONT_LABEL 14p FONT_ANNOT_PRIMARY 12p`;
`gmt psscale -Dx0.42i/4.7i/0.8i/0.12i -Ba0.2f0.1:"Z (m)": -CZ.cpt -E  -O -K >> $fileout".ps"`;
...
print PSMECA "-71.674 -31.573 22.4 353 19 83 180 71 92 8.3 0  -68.0 -29.5 Illapel (M8.3)\n";
close(PSMECA);
#-----------------------
...
`gmt pscoast -R$area -J$area_L -Df -W0.5 -N1 -Tdg-67.2/-19.5+w0.5i+f1+l",,,N" -Lg-68.5/-43+c-72.0/-30+w400k+l"km"+f -O  -K >>$fileout".ps"`;
`gmt gmtset MAP_ANNOT_OFFSET_PRIMARY 5p MAP_LABEL_OFFSET 8p FONT_LABEL 16p FONT_ANNOT_PRIMARY 12p`; #the default
#############################
...
#East/North/Up
`gmt gmtset MAP_ANNOT_OFFSET_PRIMARY 0p MAP_LABEL_OFFSET 2.5p FONT_LABEL 14p FONT_ANNOT_PRIMARY 12p`;
`gmt psbasemap -R0/510/-37.5/-25.0 -JX2.5i/3.2i -Ba200f100g100:"Time(s)":/a5f2.5:"Lat.":WSen -G255/255/255 -K -O -Y-6.2i -X4.2i >>${fileout}.ps`;
...
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
print PSTEXT "460 -34.8 10 0 1 2 ${value}m";
close(PSTEXT);
...
`cat $pred_file | awk '(NR>1 && \$1=="Illapel2015" && \$2<=${epo_sec}){print(\$2,\$8)}' | gmt psxy -R -J  -W1.2p,0/0/255 -O -K >>${fileout}.ps`;
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
...
`cat $pred_file | awk '(NR>1 && \$1=="Illapel2015" && \$2<=${epo_sec}){print(\$2,\$7)}' | gmt psxy -R -J  -W1.2p,0/0/255 -O -K >>${fileout}.ps`;
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
print PSTEXT "460 130 10 0 1 2 Length";
close(PSTEXT);

`gmt psbasemap -R0/510/16/31 -JX2.5i/0.6i -Ba200f100g100/a5f2.5Wsen -G255/255/255 -K -O -Y0.71i >>${fileout}.ps`;
#---plot real value---
open(PSXY,"|gmt psxy -R -J  -W1.5p,255/0/255,-- -O -K >>${fileout}.ps");
print PSXY "0 22.4\n";
print PSXY "510 22.4\n";
close(PSXY);
`cat $pred_file | awk '(NR>1 && \$1=="Illapel2015" && \$2<=${epo_sec}){print(\$2,\$6)}' | gmt psxy -R -J  -W1.2p,0/0/255 -O -K >>${fileout}.ps`;
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
print PSTEXT "475 18.5 10 0 1 2 Dep.";
close(PSTEXT);


`gmt psbasemap -R0/510/-35/-25 -JX2.5i/0.6i -Ba200f100g100/a5f2.5Wsen -G255/255/255 -K -O -Y0.71i >>${fileout}.ps`;
#---plot real value---
open(PSXY,"|gmt psxy -R -J  -W1.5p,255/0/255,-- -O -K >>${fileout}.ps");
print PSXY "0 -31.573\n";
print PSXY "510 -31.573\n";
close(PSXY);
`cat $pred_file | awk '(NR>1 && \$1=="Illapel2015" && \$2<=${epo_sec}){print(\$2,\$5)}' | gmt psxy -R -J  -W1.2p,0/0/255 -O -K >>${fileout}.ps`;
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
print PSTEXT "480 -32.5 10 0 1 2 Lat.";
close(PSTEXT);

`gmt psbasemap -R0/510/-74/-69 -JX2.5i/0.6i -Ba200f100g100/a2f1Wsen -G255/255/255 -K -O -Y0.71i >>${fileout}.ps`;
#---plot real value---
open(PSXY,"|gmt psxy -R -J  -W1.5p,255/0/255,-- -O -K >>${fileout}.ps");
print PSXY "0 -71.674\n";
print PSXY "510 -71.674\n";
close(PSXY);
`cat $pred_file | awk '(NR>1 && \$1=="Illapel2015" && \$2<=${epo_sec}){print(\$2,\$4)}' | gmt psxy -R -J  -W1.2p,0/0/255 -O -K >>${fileout}.ps`;
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
print PSTEXT "480 -73.0 10 0 1 2 Lon.";
close(PSTEXT);


`gmt psbasemap -R0/510/6.5/8.7 -JX2.5i/0.6i -Ba200f100g100/a1f0.5Wsen -G255/255/255 -K -O -Y0.71i >>${fileout}.ps`;
#---plot real value---
open(PSXY,"|gmt psxy -R -J  -W1.5p,255/0/255,-- -O -K >>${fileout}.ps");
print PSXY "0 8.3\n";
print PSXY "510 8.3\n";
...
close(PSXY);
#-----------------------
`cat $pred_file | awk '(NR>1 && \$1=="Illapel2015" && \$2<=${epo_sec}){print(\$2,\$3)}' | gmt psxy -R -J  -W1.2p,0/0/255 -O -K >>${fileout}.ps`;
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
print PSTEXT "480 6.8 10 0 1 2 Mw";
close(PSTEXT);
