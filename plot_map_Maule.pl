...
`gmt psbasemap -R -J -Ba5f2.5g2.5WSen -O -K >> $fileout".ps"`;
#`gmt psbasemap -R -J -Ba2f1g1WSen -O -K >> $fileout".ps"`;
...
close(PSVELO);
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
print PSTEXT "-76.1 -33.9 12 0 1 2 3m";
close(PSTEXT);
...
`gmt pscoast -R$area -JL$area_L -Df -W0.5 -N1 -Tdg-73.5/-31.0+w0.5i+f1+l",,,N" -Lg-71.1/-42.2+c-72.0/-35+w200k+l"km"+f -O  -K >>$fileout".ps"`;
`gmt gmtset MAP_ANNOT_OFFSET_PRIMARY 5p MAP_LABEL_OFFSET 8p FONT_LABEL 16p FONT_ANNOT_PRIMARY 12p`; #the default
#############################
