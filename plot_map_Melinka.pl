...
$pred_fault = `ls /Users/timlin/TEST_MLARGE/Out4/fault_${epo}.txt`; chomp($pred_fault);
...
$tmpscale=0.5*${scale_velo};
print PSVELO "-77 -25.6 $tmpscale 0 0 0 0\n";
close(PSVELO);
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
print PSTEXT "-76.3 -26.2 12 0 1 2 0.5m";
close(PSTEXT);
#---plot colorscale---
...
`cat $pred_file | awk '(NR>1 && \$1=="Melinka2016" && \$2<=${epo_sec}){print(\$2,\$8)}' | gmt psxy -R -J  -W1.2p,0/0/255 -O -K >>${fileout}.ps`;
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
...
`cat $pred_file | awk '(NR>1 && \$1=="Melinka2016" && \$2<=${epo_sec}){print(\$2,\$7)}' | gmt psxy -R -J  -W1.2p,0/0/255 -O -K >>${fileout}.ps`;
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
print PSTEXT "460 130 10 0 1 2 Length";
close(PSTEXT);

`gmt psbasemap -R0/510/25/40 -JX2.5i/0.6i -Ba200f100g100/a5f2.5Wsen -G255/255/255 -K -O -Y0.71i >>${fileout}.ps`;
#---plot real value---
open(PSXY,"|gmt psxy -R -J  -W1.5p,255/0/255,-- -O -K >>${fileout}.ps");
print PSXY "0 38\n";
print PSXY "510 38\n";
close(PSXY);
`cat $pred_file | awk '(NR>1 && \$1=="Melinka2016" && \$2<=${epo_sec}){print(\$2,\$6)}' | gmt psxy -R -J  -W1.2p,0/0/255 -O -K >>${fileout}.ps`;
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
...
open(PSXY,"|gmt psxy -R -J  -W1.5p,255/0/255,-- -O -K >>${fileout}.ps");
print PSXY "0 -43.406\n";
print PSXY "510 -43.406\n";
close(PSXY);
`cat $pred_file | awk '(NR>1 && \$1=="Melinka2016" && \$2<=${epo_sec}){print(\$2,\$5)}' | gmt psxy -R -J  -W1.2p,0/0/255 -O -K >>${fileout}.ps`;
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
print PSTEXT "480 -42.5 10 0 1 2 Lat.";
close(PSTEXT);

`gmt psbasemap -R0/510/-76/-71 -JX2.5i/0.6i -Ba200f100g100/a2f1Wsen -G255/255/255 -K -O -Y0.71i >>${fileout}.ps`;
#---plot real value---
open(PSXY,"|gmt psxy -R -J  -W1.5p,255/0/255,-- -O -K >>${fileout}.ps");
print PSXY "0 -73.941\n";
print PSXY "510 -73.941\n";
close(PSXY);
`cat $pred_file | awk '(NR>1 && \$1=="Melinka2016" && \$2<=${epo_sec}){print(\$2,\$4)}' | gmt psxy -R -J  -W1.2p,0/0/255 -O -K >>${fileout}.ps`;
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
print PSTEXT "480 -75.0 10 0 1 2 Lon.";
close(PSTEXT);
...
close(PSXY);
#-----------------------
`cat $pred_file | awk '(NR>1 && \$1=="Melinka2016" && \$2<=${epo_sec}){print(\$2,\$3)}' | gmt psxy -R -J  -W1.2p,0/0/255 -O -K >>${fileout}.ps`;
open(PSTEXT,"|gmt pstext -R -J -O -K >>$fileout'.ps'");
