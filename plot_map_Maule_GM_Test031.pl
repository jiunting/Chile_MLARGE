...
for ($epo=0;$epo<=101;$epo++){
    ...
    #------------plot P, S wavefront on the map--------------
    # run python cal_wavefront.py first!
    $Pwave = "./Maule_wavefront/P${epo}.txt";
    $Swave = "./Maule_wavefront/S${epo}.txt";
    print "Swave file=$Swave\n";
    `gmt psxy $Pwave -R -J  -W0.8p,0/0/0,-- -O -K >>$fileout".ps"`;
    `gmt psxy $Swave -R -J -W0.8p,255/0/0 -O -K >>$fileout".ps"`;
    ...
    ######Subplot 3. plot parameter predictions #######
    ...
    #======Finalize the file and make png=======
    ...
    #last; #remove this to run all loops
